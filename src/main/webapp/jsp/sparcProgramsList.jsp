...
<script>
	...
	function onSparcResultGoToDestination() {
	    ...
	    if (_isSparcPassed) {
	        ...
	        // Check if there are more learning outcomes in the current lesson
	        if (currentLearningOutcomeNumber < currentLessonDetails.rootLearningOutcome.subLearningOutcomes.length) {
	            // If there are, navigate to the next learning outcome
	            let nextLearningOutcomeNumber = currentLearningOutcomeNumber + 1;
	            let nextLearningOutcomeDetails = currentLessonDetails.rootLearningOutcome.subLearningOutcomes.find(x => x.learningOutcomeId === nextLearningOutcomeNumber);
	            let nextPageId = nextLearningOutcomeDetails.pages[0].pageId;
	            sessionStorage.setItem(sessionKeyShowPageId, nextPageId);
	            let nextLessonUrl = "../jsp/lesson" + currentLessonNumber + ".jsp";
	            window.location.href = nextLessonUrl;
	        } else {
	            // If all learning outcomes are completed, navigate to the next lesson
